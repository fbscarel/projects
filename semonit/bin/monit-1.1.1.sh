#!/usr/bin/env bash
# monit-1.1.1.sh, 2014/10/02 13:57:34 fbscarel $
...
stripdomain() {
  ...
  # remove trailing '&amp' and similar tags
  [ $( echo "$retval" | grep -oc "&" ) -ne 0 ] && retval=`echo $retval | cut -d'&' -f1`

  # remove leading 'www' and similar prefixes, return
