#!/bin/bash
# chroot-1.0.3.sh, 2014/08/20 16:04:21 fbscarel $
...
run_autoconf() {
  ...
  # check if templates exist
    ...
    echo "  [!] Template $nginx_template does not exist!"
    check_standalone $tstand
    return 1
  elif [ ! -f "$phpfpm_template" ]; then
    echo "  [!] Template $phpfpm_template does not exist!"
    check_standalone $tstand
    return 1
  fi
...
usage() {
  ...
  echo "                   [-t TEMPLATE] [-u USERNAME] [-w WEBDIR] [-y OLDHOST] [-z NEWHOST]"
  ...
  echo "load website content and write nginx/PHP-FPM configuration files automatically."
  echo
  echo "Available options:"
  echo "  -c          Only set permissions for the chroot directory. Takes no parameters,"
...
while getopts ":n:cd:hl:p:t:u:w:y:z:" opt; do
    case "$opt" in
        c)
            cflag=true
            ;;
