#!/bin/bash
# chroot-1.1.0.sh, 2014/09/25 15:26:53 fbscarel $
...
FEMAIL_PATH="$ABS_PATH/femail/femail"
...
if [ ! -z $privuser ]; then
  if [ "$( check_user $privuser )" -eq 0 ]; then
...
cp -L /lib/x86_64-linux-gnu/libtinfo.so.5           $topdir/lib/x86_64-linux-gnu/libtinfo.so.5
...
# copy & symlink femail static binary
if [ -f "$FEMAIL_PATH" ]; then
  cp -a $FEMAIL_PATH $topdir/usr/sbin/sendmail
else
  echo "  [!] femail binary not found! "
  echo -n "  [!] Press ENTER if you're OK with that, else CTRL+C and check if the FEMAIL_PATH variable is correct >> "
  echo
fi
